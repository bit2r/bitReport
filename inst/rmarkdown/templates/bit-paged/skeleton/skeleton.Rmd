...
:::{.bit-blue}
[h1 제목]{style="font-size: 2.5em;color: var(--bit-orange);"}

[h2 제목]{style="font-size: 2em;color: var(--bit-orange);"}
:::

[h3 제목]{style="font-size: 1.75em; font-weight: 700;color: var(--bit-orange);"}
...
knitr::kable(usa_crime,
             ...
             col.names = c('주 이름', '살인범수 (10만명당)', '폭행범수 (10만명당)', 
                           '도시인구 비율 (%)', '강간범수 (10만명당)'),
             format.args = list(big.mark = ",", digit = 0, scientific = FALSE),
...
화단에 [무궁화 꽃]{.bit-orange}이 활짝 피었습니다.
...
`.bit-orange`, `.bit-blue` 또는 `.bit-grey` 클래스를 추가하면 bitR 브랜드 색상을 텍스트 요소에 적용할 수 있습니다. 앞서 언급했듯이 이것은 분리된 div와 인라인 요소 모두 적용됩니다.

**Example:**

[파란색]{.bit-blue}, [오렌지색]{.bit-orange}, [회색]{.bit-grey}으로 표현됩니다. **[bold]{.bit-blue}**처럼 다른 인라인 스타일과 혼합이 가능합니다.
...
[contact2email@email.com](mailto:contact2email@email.com)
:::
